...
def test_update_exercise_with_proper_elements():
    ...
    assert exercises[0].find("skeleton").attrib["sourceDir"] == "exercises/L01/unit_converter/skeleton"
...
def test_update_attribute_returns_expected_result():
    ...
    root = tree.getroot()
    exercises = [exercise for exercise in root.iter("exercise")]
    assert ta.update_attribute(exercises[0].find("skeleton"),"sourceDir","exercises/L01/unit_converter/skeleton") == 'exercises/L01/unit_converter/skeleton'

def test_if_update_attribute_returns_expected_data_type():
    tree = te.parse("./exercise.xml")
    root = tree.getroot()
    exercises = [exercise for exercise in root.iter("exercise")]
    result = ta.update_attribute(exercises[0].find("skeleton"),"sourceDir","exercises/L01/unit_converter/skeleton")
...
def test_collect_data_returns_expected_result():
    ...
    root = tree.getroot()
    exercises = [exercise for exercise in root.iter("exercise")]
    assert ta.collect_data(exercises) == {'Převaděč jednotek': {'perex': 'nan_sourceDir', 'skeleton': 'exercises/L01/unit_converter/skeleton', 'unit-tests': 'exercises/L01/unit_converter/tests.py', 'description': 'exercises/L01/unit_converter/skeleton', 'solution': 'exercises/L01/unit_converter/solution'}}
